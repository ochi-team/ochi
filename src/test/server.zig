...
test "serverEndToEndViaHTTP:DEPRECATED" {
    const alloc = std.testing.allocator;
    const io = std.testing.io;
    ...
    {
        var resp = try ochiClient.request(alloc, .POST, "/flush", "", "", "", null);
        defer resp.deinit(alloc);
        try std.testing.expectEqual(200, resp.statusCode);
    }
...

    {
        ...
        while (true) {
            var resp = try ochiClient.request(alloc, .POST, "/query", queryJson, "default", "application/json", null);
            defer resp.deinit(alloc);
