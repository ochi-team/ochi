...
pub const DataShard = struct {
    ...
    fn flush(self: *DataShard, io: Io, alloc: Allocator, timestampsEncoders: *TimestampsEncoder.Pool, sem: *Io.Semaphore) !?*Table {
...
pub fn init(io: Io, alloc: Allocator, path: []const u8, runtime: *Runtime, timestampsEncoders: *TimestampsEncoder.Pool) !*DataRecorder {
...
fn createMemTableFromLines(io: Io, alloc: Allocator, timestampsEncoders: *TimestampsEncoder.Pool, sid: SID, lines: []Line) !*Table {
    const memTable = try MemTable.init(alloc);
    errdefer memTable.deinit(alloc);
